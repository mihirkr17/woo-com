...
const DropDown = ({ mProduct, location, productControlHandler, openDropDown }) => {
   ...
   return (
      <ul className="dropdown-menu" style={openDropDown?._LID === mProduct?._LID ? { display: 'block', right: 0 } : { display: 'none' }}>
         ...
         <li>
            <Link className='dropdown-item' state={{ from: location }} replace to={`/dashboard/manage-product?np=add-new-variation&store=${mProduct?.sellerData?.storeName}&pid=${mProduct?._id}`}>
               Add New Variation
            </Link>
         </li>
         <li>
